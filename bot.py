...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        "👋 Привет! Меня создал предприниматель, который прошёл через все трудности товарного бизнеса: от первой неудачной закупки до стабильных продаж. Я — его система, которая помогает молодым продавцам торговать легко и без лишних проблем.\n\n"
        "📌 *Что я умею:*\n"
...
async def referral_info(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"🔗 Ваша ссылка: `https://t.me/{bot.username}?start={user_id}`\n\n"
        "🤝 *Как это работает:*\n"
